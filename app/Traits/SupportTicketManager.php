...
trait SupportTicketManager
{
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        $message->admin_id            = 1 ? : authAdmin()->id;
        $message->message             = $request->message;
        $message->save();


        if ($request->hasFile('Đính kèm')) {
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        $notify[] = ['success', 'Trả lời thành công'];
    ...
    protected function storeSupportAttachments($messageId)
    {
        ...
        foreach ($this->files as  $file) {
            try {
                ...
                $notify[] = ['error', 'Tệp không thể tải lên'];
                return $notify;
            }
        }
                ...
                function ($attribute, $value, $fail) {
                    foreach ($this->files as $file) {
                        ...
                        if (($file->getSize() / 1000000) > $this->maxSize) {
                            return $fail("Tối đa $this->maxSize MB ");
                        }
                        if (!in_array($ext, $this->allowedExtension)) {
                            return $fail("Chỉ cho phép các tệp png, jpg, jpeg, pdf, doc, docx");
                        }
                    }
                    if (count($this->files) > 5) {
                        return $fail("Tối đa 5 tệp có thể được tải lên");
                    }
                },
            ],
    ...
    public function closeTicket($id)
    {
        ...
        $ticket->save();
        $notify[] = ['success', 'Đóng thành công'];
        return back()->withNotify($notify);
    }
