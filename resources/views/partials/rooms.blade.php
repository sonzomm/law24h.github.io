...
{{--                        @php $selectedRoom = 0; @endphp--}}
                        ...
                        @foreach ($rooms as $room)
                            ...
                            @if ($booked)
                                <button class="btn btn--danger btn-sm room-btn" style="border-radius: 50%" room="room-{{ $room->room_number }}" disabled>{{ $room->room_number }}</button>
                                @php
                                    $bookedRooms[$room->room_number]++;
                                @endphp
                            @else
                                <button class="btn btn--primary btn-sm room-btn available" style="border-radius: 50%" room="room-{{ $room->room_number }}" data-room="{{ $room }}" data-date="{{ $checkIn->format('m/d/Y') }}" data-booked_status="0">
                                    {{ $room->room_number }}
                                </button>
{{--                                @php--}}
{{--                                    $selectedRoom++;--}}
    ...
    (function($) {
        ...
        function enableDisableBooking() {
            ...
            if (needChanged) {
                ...
                alertBox.html('@lang("Phòng không khớp")');
            }
            ...
            if (limitCross) {
                ...
                alertBox.html('@lang("SSố lượng phòng Không được lớn hơn ")' + numberOfRooms + " trong mỗi ngày");
            }
            if (lowFromLimit) {
                if (!alertBox.hasClass('alert-danger')) {
                    alertBox.removeClass('alert-info').addClass('alert-danger');
                ...
                alertBox.removeClass('alert-info').addClass('alert-danger');
                alertBox.html('@lang("Số lượng phòng Không được nhỏ hơn ")' + numberOfRooms + " trong mỗi ngày");
            }
