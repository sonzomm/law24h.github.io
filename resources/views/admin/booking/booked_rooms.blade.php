        ...
        <div class="col-lg-12">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <div class="d-flex flex-wrap gap-3">
                    ...
                    @if ($bookedBy)
                        <span>
                            <span>@lang('Đã được đặt bởi')</span>:
                    ...
                    @if ($approvedBy)
                        <span>
                            <span>@lang('Được phê duyệt bởi')</span>:
                    ...
                    @if ($checkedOutBy)
                        <span>
                            <span>@lang('Checked Out bởi')</span>:
                        ...
                        <table class="table--light style--two table">
                            ...
                            <tbody>
                                @forelse($bookedRooms as $key => $bookedRoom)
                                    <tr>
                                        @php
                                            $cancellationFee = $bookedRoom->where('status', \App\Constants\Status::ROOM_ACTIVE)->sum('cancellation_fee');
                                            $totalFare = $bookedRoom->where('status', \App\Constants\Status::ROOM_ACTIVE)->sum('fare');
                                            $shouldRefund = $totalFare - $cancellationFee;
                                            $activeBooking = $bookedRoom->where('status', \App\Constants\Status::ROOM_ACTIVE)->count();
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-end flex-wrap gap-2">
                                                @foreach ($bookedRoom as $item)
                                                    @if ($item->status == \App\Constants\Status::BOOKED_ROOM_CANCELED)
                                                    ...
                                                    @elseif($item->status == \App\Constants\Status::BOOKED_ROOM_CHECKOUT)
                                                    ...
                                                    @elseif($item->status == \App\Constants\Status::BOOKED_ROOM_ACTIVE)
                                                        <div class="bg--18 room-container rounded p-2">
                                                            ...
                                                            @if (now()->toDateString() <= $item->booked_for)
                                                                @can('admin.booking.booked.room.cancel')
                                                                    <button class="cancel-btn cancelBookingBtn" data-fare="{{ showAmount($item->fare) }} {{($general->cur_text) }}" data-id="{{ $item->id }}" data-room_number="{{ $item->room->room_number }}" data-should_refund="{{ showAmount($item->fare - $item->cancellation_fee) }} {{($general->cur_text) }}" type="button"><i class="las la-times"></i>
                                                                    </button>
                                                                @endcan
                                                            @endif
                                                        </div>
