        ...
        <div class="container">
            <div class="row g-4 justify-content-center">
                <div class="col-lg-8">
                    <div class="room-details-head">
                        <div>
                            ...
                            <div class="d-flex justify-content-center flex-wrap gap-3">
                                <span>
                                    @lang('Người lớn') &nbsp; {{ $roomType->total_adult }}
                                </span>

                                <span>
                                    @lang('Trẻ em') &nbsp; {{ $roomType->total_child }}
                                </span>
                            </div>
                        </div>

                        <div>
                            <h2 class="text--base fare">{{ showAmount($roomType->fare)}}{{($general->cur_sym) }}</h2>
                            <span class="text--small">+{{ $general->tax }}% {{($general->tax_name) }}</span>
                            <span class="text--base text-sm"> / @lang('Đêm')</span>
                        </div>
                    </div>
                    ...
                    <div class="room-details-card mt-4">
                        <h5 class="title">@lang('Chính sách hủy')</h5>

                        <div class="body">
                            @if ($roomType->cancellation_fee == 0)
                                <span> <i class="las la-check-double"></i> @lang('Hủy miễn phí')</span>
                            @else
                                <h5 class="text-center">@lang('Phí hủy') {{showAmount($roomType->cancellation_fee).$general->cur_sym}} / @lang('Night')</h5>
                            @endif
                    ...
                    @if ($roomType->amenities)
                        <div class="room-details-card mt-4">
                            <h5 class="title">@lang('Tiện ích')</h5>
                    ...
                    @if ($roomType->complements)
                        <div class="room-details-card mt-4">
                            <h5 class="title">@lang('Dịch vụ đi kèm')</h5>
                            <div class="body">
                                <div class="d-inline-flex flex-md-row flex-column gap-md-5 flex-wrap gap-3">
                    ...
                    @if ($roomType->beds)
                        <div class="room-details-card mt-4">
                            <h5 class="title">@lang('Số giường')</h5>
                            <div class="body">
                                <div class="d-inline-flex flex-md-row flex-column gap-md-5 flex-wrap gap-3">
                    ...
                    <div class="room-booking-sidebar">
                        <div class="room-booking-widget">
                            <div class="room-booking-widget__body mt-0">
                                ...
                                <div class="mb-3">
                                    <label class="fw-bold">@lang('Phòng')</label>
        ...
        (function($) {
            ...
            $('.booking').on('click', function() {
                ...
                if (minCheckIn >= maxCheckOut) {
                    notify('error', 'Ngày Check in phải lớn hơn ngày trả phòng!')
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            messageBox.text(`@lang('Bạn có thể đặt ${response.success} phòng')`);
            ...
            $('[name=number_of_rooms]').on('input', function() {
                ...
                if ($(this).val() > maxRoomBookingLimit) {
                    btnRequest.attr('disabled', true);
                    notify('error', "Số phòng không được lớn hơn phòng tối đa cho phép");
                }
            });
